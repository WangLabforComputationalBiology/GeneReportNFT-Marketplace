...
import {GeneSharing} from "./GeneSharing.sol";
...
contract SharingPlatform is ERC20 {
    ...
    // sharingID 到合约地址的映射
    mapping(string GeneSharingID => address) private _sharingIDToContract;

    // 用户创建的GeneSharing的数目
    mapping(address => uint256) private _sharingAmount;

    // 机构认证 KYC 状态
    mapping(address => bool) private _isAuthed;

    // 新建GeneSharing合约事件
    event GeneSharingCreated(string indexed sharingID, address indexed contractAddress, address creator, bool isOfficial);
...
//    }
    ...
    function createAllFromThirdParty(address user,string calldata sharingID, bytes32[] calldata dataHashs) external onlyAdmin returns (address) {
        require(_sharingIDToContract[sharingID] == address(0), "SharingID exists");

        GeneSharing newSharing = new GeneSharing(sharingID,address(this),true);

        address contractAddress = address(newSharing);
        _geneSharingContracts.push(contractAddress);
        _sharingIDToContract[sharingID] = contractAddress;
        _sharingAmount[user]++;

        emit GeneSharingCreated(sharingID, contractAddress, msg.sender, true);
    ...
    function createEmptyGeneSharingFromCreativeWorkSpace(string memory sharingID) external returns (address) {
        require(_sharingIDToContract[sharingID] == address(0), "SharingID exists");
        GeneSharing newSharing = new GeneSharing(sharingID,address(this),false);
        address contractAddress = address(newSharing);
        _geneSharingContracts.push(contractAddress);
        _sharingIDToContract[sharingID] = contractAddress;
        emit GeneSharingCreated(sharingID, contractAddress, msg.sender, false);
        _sharingAmount[msg.sender]++;
        return contractAddress;
    }

    // 添加多个Metadata
    function addMetadataBatchesFromCreativeWorkSpace(string memory sharingID, bytes32[] calldata dataHash) external returns(address){
        require(_isAuthed[msg.sender], "KYC not verified");
        require(_sharingIDToContract[sharingID] != address(0), "Sharing not found");
        GeneSharing geneSharing = GeneSharing(_sharingIDToContract[sharingID]);
        Metadata metadata=Metadata(_metadataContract);
    ...
    //新增查看权限
    function addViewAccess(string memory sharingID, bytes32 dataHash,string calldata remark) external {
        GeneSharing geneSharing = GeneSharing(_sharingIDToContract[sharingID]);
        Metadata metadata = Metadata(_metadataContract);
